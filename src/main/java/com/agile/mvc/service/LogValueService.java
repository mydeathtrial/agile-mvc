...
public class LogValueService extends BusinessService<LogValueEntity> {
    ...
    @ApiOperation(value = "更新[系统管理]日志相关字段值变动信息", httpMethod = "UPDATE", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "entity", value = "实体", paramType = "body", dataType = "LogValueEntity"),
            @ApiImplicitParam(name = "id", value = "唯一标识", paramType = "path", dataType = "String")
    })
    ...
    @ApiOperation(value = "[系统管理]日志相关字段值变动信息分页查询", httpMethod = "GET", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
