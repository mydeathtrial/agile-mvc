...
public class LogValueService extends BusinessService<LogValueEntity> {
    ...
    @Mapping(value = "/log-value", method = RequestMethod.PUT)
    public RETURN customUpdate() throws NoSuchIDException, IllegalAccessException {
        return super.update();
    }

    @ApiOperation(value = "分页查询[系统管理]日志相关字段值变动信息", httpMethod = "POST", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "entity", value = "实体", paramType = "body", dataType = "LogValueEntity"),
    ...
    @Models({LogValueEntity.class})
    @Mapping(path = "/log-value/list/query", method = RequestMethod.POST)
    @Validates({
    ...
    @ApiOperation(value = "查询[系统管理]日志相关字段值变动信息", httpMethod = "POST", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
        @ApiImplicitParam(name = "entity", value = "实体", paramType = "body", dataType = "LogValueEntity")
    })
    @Models({LogValueEntity.class})
    @Mapping(path = "/log-value/query", method = RequestMethod.POST)
