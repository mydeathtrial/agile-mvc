...
public class SysBtUsersRolesService extends BusinessService<SysBtUsersRolesEntity> {
    ...
    @Mapping(value = "/sys-bt-users-roles", method = RequestMethod.PUT)
    public RETURN customUpdate() throws NoSuchIDException, IllegalAccessException {
        return super.update();
    }

    @ApiOperation(value = "分页查询[系统管理]用户角色表", httpMethod = "POST", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "entity", value = "实体", paramType = "body", dataType = "SysBtUsersRolesEntity"),
    ...
    @Models({SysBtUsersRolesEntity.class})
    @Mapping(path = "/sys-bt-users-roles/list/query", method = RequestMethod.POST)
    @Validates({
    ...
    @ApiOperation(value = "查询[系统管理]用户角色表", httpMethod = "POST", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
        @ApiImplicitParam(name = "entity", value = "实体", paramType = "body", dataType = "SysBtUsersRolesEntity")
    })
    @Models({SysBtUsersRolesEntity.class})
    @Mapping(path = "/sys-bt-users-roles/query", method = RequestMethod.POST)
