...
@Mapping("/api/SysBtTaskTargetService")
@Service
public class SysBtTaskTargetService extends BusinessService<SysBtTaskTargetEntity> {
    ...
    @ApiOperation(value = "更新[系统管理]定时任务目标任务表", httpMethod = "UPDATE", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "entity", value = "实体", paramType = "body", dataType = "SysBtTaskTargetEntity"),
            @ApiImplicitParam(name = "id", value = "唯一标识", paramType = "path", dataType = "String")
    })
    ...
    @ApiOperation(value = "[系统管理]定时任务目标任务表分页查询", httpMethod = "GET", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
