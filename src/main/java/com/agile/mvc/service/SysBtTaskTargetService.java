...
public class SysBtTaskTargetService extends BusinessService<SysBtTaskTargetEntity> {
    ...
    @Mapping(value = "/sys-bt-task-target", method = RequestMethod.PUT)
    public RETURN customUpdate() throws NoSuchIDException, IllegalAccessException {
        return super.update();
    }

    @ApiOperation(value = "分页查询[系统管理]定时任务目标任务表", httpMethod = "POST", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "entity", value = "实体", paramType = "body", dataType = "SysBtTaskTargetEntity"),
    ...
    @Models({SysBtTaskTargetEntity.class})
    @Mapping(path = "/sys-bt-task-target/list/query", method = RequestMethod.POST)
    @Validates({
    ...
    @ApiOperation(value = "查询[系统管理]定时任务目标任务表", httpMethod = "POST", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiImplicitParams({
        @ApiImplicitParam(name = "entity", value = "实体", paramType = "body", dataType = "SysBtTaskTargetEntity")
    })
    @Models({SysBtTaskTargetEntity.class})
    @Mapping(path = "/sys-bt-task-target/query", method = RequestMethod.POST)
